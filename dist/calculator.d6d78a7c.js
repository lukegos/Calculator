...
var screenText = document.querySelector('.output_number_text');
var cachedNum = null;
var operator = "";

function typeNum(numberr) {
  if (screenText.innerText.length >= 17) {} else if (screenText.innerText === "0") {
    screenText.innerText = numberr;
  } else {
    screenText.innerText += numberr; //= screenText.innerText + '' + numberr;
  }
...
document.querySelector('.container').addEventListener('click', function (event) {
  if (event.target.tagName === 'BUTTON' && event.target.classList.contains("equal")) {
    if (cachedNum === null) {} else {
      ...
      var parseDisp = parseInt(screenText.innerText);
      var result = 0; //performing operation
